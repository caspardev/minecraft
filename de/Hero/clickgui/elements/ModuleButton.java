...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Enigma.instance.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Enigma.instance.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if(Enigma.instance.settingsManager.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Enigma.instance.clickGui.closeAllSettings();
				this.extended = b;
